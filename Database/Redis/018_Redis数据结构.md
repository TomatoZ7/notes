...
- [Redis 数据结构](#redis-数据结构)
  ...
  - [8.quicklist](#8quicklist)
    - [8.1 quicklist 结构设计](#81-quicklist-结构设计)
  - [9.listpack](#9listpack)
    - [9.1 listpack 结构设计](#91-listpack-结构设计)
  - [10.Q&A](#10qa)
    - [10.1 压缩列表的 entry 为什么要保存 prevlen 呢？listpack 改成 len 之后不会影响功能吗？](#101-压缩列表的-entry-为什么要保存-prevlen-呢listpack-改成-len-之后不会影响功能吗)
  - [11.参考](#11参考)
...
![image](Images/data_structure_32.png)
...
![image](Images/data_structure_33.png)
...
![image](Images/data_structure_34.png)

`listpack` 头包含两个属性，分别记录了 `listpack` 总字节数和元素数量，然后 `listpack` 末尾也有个结尾标识。图中的 `listpack entry` 就是 `listpack` 的节点了。

每个 `listpack` 节点结构如下：

![image](Images/data_structure_35.png)
...
### 10.1 压缩列表的 entry 为什么要保存 prevlen 呢？listpack 改成 len 之后不会影响功能吗？

压缩列表的 `entry` 保存 `prevlen` 是为了实现节点从后往前遍历，知道前一个节点的长度，就可以计算前一个节点的偏移量。

`listpack` 一样可以支持从后往前遍历的。详细的算法可以看：[https://github.com/antirez/listpack/blob/master/listpack.c](https://github.com/antirez/listpack/blob/master/listpack.c) 里的 `lpDecodeBacklen` 函数，`lpDecodeBacklen` 函数就可以从当前列表项起始位置的指针开始，向左逐个字节解析，得到前一项的 `entry - len` 值。
