...
- [揭开 Buffer Pool 的面纱](#揭开-buffer-pool-的面纱)
  - [1.为什么要有 Buffer Pool？](#1为什么要有-buffer-pool)
  - [2.Buffer Pool 有多大？](#2buffer-pool-有多大)
  - [3.Buffer Pool 缓存什么？](#3buffer-pool-缓存什么)
  - [4.如何管理 Buffer Pool？](#4如何管理-buffer-pool)
    - [4.1 如何管理空闲页？](#41-如何管理空闲页)
    - [4.2 如何管理脏页？](#42-如何管理脏页)
    - [4.3 如何提高缓存命中率？](#43-如何提高缓存命中率)
      - [4.3.1 什么是预读失效？](#431-什么是预读失效)
      - [4.3.2 什么是 Buffer Pool 污染？](#432-什么是-buffer-pool-污染)
    - [4.4 脏页什么时候会被刷入磁盘？](#44-脏页什么时候会被刷入磁盘)
  - [5.总结](#5总结)
  - [6.参考](#6参考)
...
![image](Images/buffer_pool_1.webp)
...
![image](Images/buffer_pool_4.webp)
...
![image](Images/buffer_pool_5.webp)
...
![image](Images/buffer_pool_10.webp)
...
![image](Images/buffer_pool_11.webp)

现在有个编号为 20 的页被预读了，这个页只会被插入到 old 区域头部，而 old 区域末尾的页（10号）会被淘汰掉。

![image](Images/buffer_pool_12.webp)

如果 20 号页一直不会被访问，它也没有占用到 young 区域的位置，而且还会比 young 区域的数据更早被淘汰出去。

如果 20 号页被预读后，立刻被访问了，那么就会将它插入到 young 区域的头部，young 区域末尾的页（7号），会被挤到 old 区域，作为 old 区域的头部，这个过程并不会有页被淘汰。

![image](Images/buffer_pool_13.webp)

虽然通过划分 old 区域 和 young 区域避免了预读失效带来的影响，但是还有个问题无法解决，那就是 Buffer Pool 污染的问题。

#### 4.3.2 什么是 Buffer Pool 污染？

当某一个 SQL 语句**扫描了大量的数据**时，在 Buffer Pool 空间比较有限的情况下，可能会将 **Buffer Pool 里的所有页都替换出去，导致大量热数据被淘汰了**，等这些热数据又被再次访问的时候，由于缓存未命中，就会产生大量的磁盘 I/O，MySQL 性能就会急剧下降，这个过程被称为 **Buffer Pool 污染**。
...
![image](Images/buffer_pool_14.webp)

在批量访问这些数据的时候，会被逐一插入到 young 区域头部。

![image](Images/buffer_pool_15.webp)
...
+ 当**页被访问且 old 区域停留时间超过 innodb_old_blocks_time 阈值（默认为 1 秒）**时，才会将页插入到 young 区域，否则还是插入到 old 区域，目的是为了解决批量数据访问，大量热数据淘汰的问题。
