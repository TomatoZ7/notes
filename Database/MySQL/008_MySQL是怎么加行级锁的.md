...
- [MySQL是怎么加行级锁的？](#mysql是怎么加行级锁的)
  - [1.什么 SQL 语句会加行级锁？](#1什么-sql-语句会加行级锁)
  - [2.行级锁的分类](#2行级锁的分类)
  - [3.MySQL 是怎么加行级锁的？](#3mysql-是怎么加行级锁的)
    - [3.1 唯一索引等值查询](#31-唯一索引等值查询)
      - [3.1.1 记录存在的情况](#311-记录存在的情况)
        - [有什么命令可以分析加了什么锁？](#有什么命令可以分析加了什么锁)
        - [为什么唯一索引等值查询并且查询记录存在的场景下，该记录的索引中的 next-key lock 会退化成记录锁？](#为什么唯一索引等值查询并且查询记录存在的场景下该记录的索引中的-next-key-lock-会退化成记录锁)
      - [3.1.2 记录不存在的情况](#312-记录不存在的情况)
        - [间隙锁的范围(1, 5) ，是怎么确定的？](#间隙锁的范围1-5-是怎么确定的)
        - [为什么唯一索引等值查询并且查询记录「不存在」的场景下，在索引树找到第一条大于该查询记录的记录后，要将该记录的索引中的 next-key lock 会退化成「间隙锁」？](#为什么唯一索引等值查询并且查询记录不存在的场景下在索引树找到第一条大于该查询记录的记录后要将该记录的索引中的-next-key-lock-会退化成间隙锁)
    - [3.2 唯一索引范围查询](#32-唯一索引范围查询)
      - [3.2.1 针对「大于或者大于等于」的范围查询](#321-针对大于或者大于等于的范围查询)
        - [针对「大于」的范围查询的情况。](#针对大于的范围查询的情况)
        - [针对「大于等于」的范围查询的情况。](#针对大于等于的范围查询的情况)
      - [3.2.2 针对「小于或者小于等于」的范围查询](#322-针对小于或者小于等于的范围查询)
        - [实验一：针对「小于」的范围查询时，查询条件值的记录「不存在」表中的情况。](#实验一针对小于的范围查询时查询条件值的记录不存在表中的情况)
        - [实验二：针对「小于等于」的范围查询时，查询条件值的记录「存在」表中的情况。](#实验二针对小于等于的范围查询时查询条件值的记录存在表中的情况)
        - [实验三：再来看针对「小于」的范围查询时，查询条件值的记录「存在」表中的情况。](#实验三再来看针对小于的范围查询时查询条件值的记录存在表中的情况)
    - [3.3 非唯一索引等值查询](#33-非唯一索引等值查询)
      - [3.3.1 记录不存在的情况](#331-记录不存在的情况)
        - [实验一：针对非唯一索引等值查询时，查询的值不存在的情况。](#实验一针对非唯一索引等值查询时查询的值不存在的情况)
        - [实验二：针对非唯一索引等值查询时，查询的值存在的情况。](#实验二针对非唯一索引等值查询时查询的值存在的情况)
    - [3.4 非唯一索引范围查询](#34-非唯一索引范围查询)
    - [3.5 没有加索引的查询](#35-没有加索引的查询)
  - [4.总结](#4总结)
  - [5.参考](#5参考)
...
关于行级锁的分类可以查看这篇文章：[行级锁分类](007_MySQL有哪些锁.md#3行级锁)
...
![image](Images/how_to_lock_1.webp)
...
![image](Images/how_to_lock_4.webp)
...
![image](Images/how_to_lock_5.webp)
...
![image](Images/how_to_lock_6.webp)
...
![image](Images/how_to_lock_7.webp)
...
![image](Images/how_to_lock_8.webp)
...
![image](Images/how_to_lock_9.webp)
...
![image](Images/how_to_lock_10.webp)
...
![image](Images/how_to_lock_12.webp)
...
![image](Images/how_to_lock_13.webp)
...
![image](Images/how_to_lock_14.webp)
...
![image](Images/how_to_lock_15.webp)
...
![image](Images/how_to_lock_16.webp)
...
![image](Images/how_to_lock_17.webp)
...
![image](Images/how_to_lock_18.webp)

我们也可以通过 `select * from performance_schema.data_locks\G;` 这条语句来看看事务 A 加了什么锁。

输出结果如下，我这里只截取了行级锁的内容。

![image](Images/how_to_lock_19.webp)
...
![image](Images/how_to_lock_20.webp)
...
![image](Images/how_to_lock_21.webp)
...
![image](Images/how_to_lock_22.webp)
...
+ 由于是范围查询，接着继续扫描已经存在的二级索引记录。扫描的第二行是 age = 39 的二级索引记录，于是对该二级索引记录加 next-key 锁，范围是 (22, 39]，同时，对 age = 39 这条记录的主键索引加记录锁，即对 id = 20 这一行记录的主键索引加记录锁。
...
![image](Images/how_to_lock_23.webp)
