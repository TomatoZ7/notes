...
- [1.MySQL 有哪些锁？](#1mysql-有哪些锁)
  - [1.全局锁](#1全局锁)
    - [1.1 全局锁是怎么用的？](#11-全局锁是怎么用的)
    - [1.2 全局锁应用场景是什么？](#12-全局锁应用场景是什么)
    - [1.3 加全局锁又会带来什么缺点呢？](#13-加全局锁又会带来什么缺点呢)
    - [1.4 既然备份数据库数据的时候，使用全局锁会影响业务，那有什么其他方式可以避免？](#14-既然备份数据库数据的时候使用全局锁会影响业务那有什么其他方式可以避免)
  - [2.表锁](#2表锁)
    - [2.1 表锁](#21-表锁)
    - [2.2 元数据锁](#22-元数据锁)
      - [2.2.1 MDL 不需要显示调用，那它是在什么时候释放的?](#221-mdl-不需要显示调用那它是在什么时候释放的)
      - [2.2.2 为什么线程 C 因为申请不到 MDL 写锁，而导致后续的申请读锁的查询操作也会被阻塞？](#222-为什么线程-c-因为申请不到-mdl-写锁而导致后续的申请读锁的查询操作也会被阻塞)
    - [2.3 意向锁](#23-意向锁)
    - [2.4 AUTO-INC 锁](#24-auto-inc-锁)
  - [3.行级锁](#3行级锁)
    - [3.1 Record Lock](#31-record-lock)
    - [3.2 Gap Lock](#32-gap-lock)
    - [3.3 Next-Key Lock](#33-next-key-lock)
    - [3.4 插入意向锁](#34-插入意向锁)
  - [4.参考](#4参考)
...
### 1.4 既然备份数据库数据的时候，使用全局锁会影响业务，那有什么其他方式可以避免？
...
#### 2.2.2 为什么线程 C 因为申请不到 MDL 写锁，而导致后续的申请读锁的查询操作也会被阻塞？
...
![image](Images/lock_6.webp)

当事务 A 还没提交的时候，事务 B 向该表插入一条 id = 4 的新记录，这时会判断到插入的位置已经被事务 A 加了间隙锁，于是事务 B 会生成一个插入意向锁，然后将锁的状态设置为等待状态，此时事务 B 就会发生阻塞，直到事务 A 提交了事务。
