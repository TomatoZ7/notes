...
- [MySQL 日志：undo log、redo log、binlog 有什么用？](#mysql-日志undo-logredo-logbinlog-有什么用)
  ...
  - [3.redo log](#3redo-log)
    ...
    - [3.4 redo log 要写到磁盘，数据也要写磁盘，为什么要多此一举？](#34-redo-log-要写到磁盘数据也要写磁盘为什么要多此一举)
    - [3.5 产生的 redo log 是直接写入磁盘的吗？](#35-产生的-redo-log-是直接写入磁盘的吗)
    - [3.6 redo log 什么时候刷盘？](#36-redo-log-什么时候刷盘)
    - [3.7 redo log 文件写满了怎么办？](#37-redo-log-文件写满了怎么办)
  - [4.binlog](#4binlog)
    - [4.1 为什么有了 binlog，还要有 redo log？](#41-为什么有了-binlog还要有-redo-log)
    - [4.2 redo log 和 binlog 有什么区别？](#42-redo-log-和-binlog-有什么区别)
      - [4.2.1 适用对象不同](#421-适用对象不同)
      - [4.2.2 文件格式不同](#422-文件格式不同)
      - [4.2.3 写入方式不同](#423-写入方式不同)
      - [4.2.4 用途不同](#424-用途不同)
    - [4.3 数据库被删除了，用 redo log / binlog 恢复？](#43-数据库被删除了用-redo-log--binlog-恢复)
    - [4.4 主从复制是怎么实现？](#44-主从复制是怎么实现)
      - [4.4.1 从库是不是越多越好？](#441-从库是不是越多越好)
      - [4.4.2 MySQL 主从复制还有哪些模型？](#442-mysql-主从复制还有哪些模型)
    - [4.5 binlog 什么时候刷盘](#45-binlog-什么时候刷盘)
      - [4.5.1 什么时候 binlog cache 会写到 binlog 文件？](#451-什么时候-binlog-cache-会写到-binlog-文件)
  - [6.两阶段提交](#6两阶段提交)
    - [6.1 两阶段提交的过程是怎样的？](#61-两阶段提交的过程是怎样的)
    - [6.2 异常重启会出现什么现象？](#62-异常重启会出现什么现象)
    - [6.4 为什么 prepare 阶段的 redo log 加上完整 binlog，重启就会提交？](#64-为什么-prepare-阶段的-redo-log-加上完整-binlog重启就会提交)
    - [6.5 事务没提交的时候，redo log 会被持久化到磁盘吗？](#65-事务没提交的时候redo-log-会被持久化到磁盘吗)
  - [7.两阶段提交有什么问题？](#7两阶段提交有什么问题)
    - [7.1 为什么两阶段提交的磁盘 I/O 次数会很高？](#71-为什么两阶段提交的磁盘-io-次数会很高)
    - [7.2 为什么锁竞争激烈？](#72-为什么锁竞争激烈)
    - [7.3 组提交](#73-组提交)
    - [7.4 redo log 有组提交吗？](#74-redo-log-有组提交吗)
      - [7.4.1 flush 阶段](#741-flush-阶段)
      - [7.4.2 sync 阶段](#742-sync-阶段)
      - [7.4.3 commit 阶段](#743-commit-阶段)
  - [8.MySQL 磁盘 I/O 很高，有什么优化的方法？](#8mysql-磁盘-io-很高有什么优化的方法)
  - [9.总结](#9总结)
  - [10.参考](#10参考)
...
### 3.2 被修改 Undo 页面，需要记录对应 redo log 吗？
...
![image](Images/log_6.webp)

所以有了 redo log，再通过 WAL 技术，InnoDB 就可以保证即使数据库发生异常重启，之前已提交的记录都不会丢失，这个能力称为 **crash-safe**（崩溃恢复）。可以看出来，**redo log 保证了事务四大特性中的持久性**。

### 3.4 redo log 要写到磁盘，数据也要写磁盘，为什么要多此一举？
...
### 3.5 产生的 redo log 是直接写入磁盘的吗？

不是的。

实际上， 执行一个事务的过程中，产生的 redo log 也不是直接写入磁盘的，因为这样会产生大量的 I/O 操作，而且磁盘的运行速度远慢于内存。

所以，redo log 也有自己的缓存 —— **redo log buffer**，每当产生一条 redo log 时，会先写入到 redo log buffer，后续再持久化到磁盘如下图：
...
+ 当设置该**参数为 1 时**，表示每次事务提交时，都将**缓存在 redo log buffer 里的 redo log 直接持久化到磁盘**，这样可以保证 MySQL 异常重启之后数据不会丢失。
...
### 4.1 为什么有了 binlog，还要有 redo log？

这个问题跟 MySQL 的时间线有关系。

最开始 MySQL 里并没有 InnoDB 引擎，MySQL 自带的引擎是 MyISAM，但是 MyISAM 没有 crash-safe（崩溃恢复） 的能力，binlog 日志只能用于归档。
...
### 4.3 数据库被删除了，用 redo log / binlog 恢复？
...
### 6.4 为什么 prepare 阶段的 redo log 加上完整 binlog，重启就会提交？

binlog 已经写入了，之后就会被从库（或者用这个 binlog 恢复出来的库）使用。

所以，在主库上也要提交这个事务。采用这个策略，主库和备库的数据就保证了一致性。

### 6.5 事务没提交的时候，redo log 会被持久化到磁盘吗？
...
+ 将 `innodb_flush_log_at_trx_commit` 设置为 2。表示每次事务提交时，都只是将缓存在 redo log buffer 里的 redo log 写到 redo log 文件，注意写入到「redo log 文件」并不意味着写入到了磁盘，因为操作系统的文件系统中有个 Page Cache，专门用来缓存文件数据的，所以写入「redo log 文件」意味着写入到了操作系统的文件缓存，然后交由操作系统控制持久化到磁盘的时机。但是这样做的风险是，主机掉电的时候会丢数据。
