...
- [count(*) 和 count(1) 有什么区别？哪个性能最好？](#count-和-count1-有什么区别哪个性能最好)
  - [1.结论](#1结论)
  - [2.count() 是什么？](#2count-是什么)
    - [2.1 count(主键字段) 执行过程是怎样的？](#21-count主键字段-执行过程是怎样的)
    - [2.2 count(1) 执行过程是怎样的？](#22-count1-执行过程是怎样的)
    - [2.3 count(*) 执行过程是怎样的？](#23-count-执行过程是怎样的)
    - [2.4 count(字段) 执行过程是怎样的？](#24-count字段-执行过程是怎样的)
    - [2.5.小结](#25小结)
  - [3.为什么要通过遍历的方式来计数？](#3为什么要通过遍历的方式来计数)
  - [4.如何优化 count(*)？](#4如何优化-count)
    - [4.1 近似值](#41-近似值)
    - [4.2 额外表保存计数值](#42-额外表保存计数值)
  - [5.参考](#5参考)
