...
![map_search_1](../Images/map_search_1.png)
...
bucket定位：**哈希值的低 B 个 bit 位**，用来定位key所存放的bucket
...
tophash值定位：**哈希值的高 8 个 bit 位**，用来快速判断 key 是否已在当前 bucket 中（如果不在的话，需要去 bucket 的 overflow 中查找）

用步骤2中的 hash 值，得到高 8 个 bit 位，也就是 `10010111`，转化为十进制，也就是 151
...
![map_search_2](../Images/map_search_2.png)
